...
/**
 * Auto-generate articles script for andrad.ro
...
function runCommand(command, args, cwd) {
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn(actualCommand, args, {
      ...
      env: {
        ...process.env,
        PATH: `${NODE_BIN_DIR}:${process.env.PATH || ''}` // Add node bin to PATH for npm/npx
      }
    });
    proc.on('close', (code) => {
      if (code === 0) resolve();
...
// Select keywords rotating through categories daily
...
async function main() {
  ...
  await log(`Will generate: ${Math.min(ARTICLES_PER_RUN, pendingKeywords.length)} articles from different categories`);

  // Select keyword from next category in rotation
  ...
  // Update generate-articles.js with these keywords
  ...
  // Write temporary articles config
  ...
  // Run related articles script
  try {
    await runCommand('node', ['scripts/refresh-related.js'], projectDir);
    await log('Related articles updated');
  } catch (error) {
    await log(`Warning: refresh-related failed: ${error.message}`);
  }

  // Read successful keywords from generate-batch.js output
  ...
  // Remove processed keywords from pending, add failed ones back at the end
  ...
  const projectName = process.env.CLOUDFLARE_PROJECT_NAME || 'andrad-ro';
...
// Generate image prompt based on keyword and category
function generateImagePrompt(keyword, category) {
  const basePrompts = {
    'Tehnologie & Gaming': 'Ultra-realistic photo of premium gaming/tech equipment, modern minimalist desk setup, RGB lighting, soft studio lighting, high detail texture, sharp focus, professional product photography, no people, neutral background, realistic proportions',
    'Electrocasnice': 'Ultra-realistic photo of premium home appliance, sleek modern design, placed in bright contemporary kitchen or home, soft natural daylight, clean reflections, high detail texture, sharp focus, professional product photography, no people, neutral background, realistic proportions',
    'TV & Audio': 'Ultra-realistic photo of premium audio/visual equipment, modern living room setting, soft ambient lighting, high detail texture, sharp focus, professional product photography, luxury entertainment device, no people, neutral background, realistic proportions',
    'Foto & Video': 'Ultra-realistic photo of professional photography/videography equipment, sleek modern design, soft studio lighting, high detail texture, sharp focus, professional product photography, premium quality device, no people, neutral background, realistic proportions',
    'Smart Home': 'Ultra-realistic photo of smart home device, modern minimalist design, contemporary home setting, soft natural daylight, clean reflections, high detail texture, sharp focus, professional product photography, smart technology, no people, neutral background, realistic proportions'
  };

  const base = basePrompts[category] || basePrompts['Tehnologie & Gaming'];
  return `${keyword} - ${base}`;
}
